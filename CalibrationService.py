...
import single_rateCallibration
...
def CalibrationService(pid=None):
    ...
    # SAM 27DEC21 : Added this line to run the calibration in loop for all active hotels where run_calibration = YES
    if pid == None:
        myquery = "SELECT id, capacity, property_name as pname FROM clients where status = 'active' and run_calibration = 'yes'"
    else:
    ...
    if pid == None:
        for row in hCapacity.itertuples():
            hCap = row.capacity
            pid = row.id
            logger.info("The Hotel {} PID - {} Capacity - {} ".format(row.pname, pid, hCap))
            try:
                logger.info("--- Run Calibration ---")
                rcpalgo.rcpalgo(pid=pid, hCap=hCap, db_config=config)
                single_rateCallibration.run_single_rateCallibration(cid=pid,config=config)
                return 'Success'
            except Exception as E:
