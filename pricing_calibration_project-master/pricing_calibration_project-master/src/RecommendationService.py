...
def getDBConfig(logger=None, cStr=0, cArray=0):
    ...
    try:
        if logger != None: logger.info("Reading setup file")
        configParser.read(configFilePath)
    except:
        # return ('file not available')
        if logger != None: logger.error('file not available')

    try:
        if logger != None: logger.info("Got credentials for DB connection")
    ...
    if cArray == 1:
        aConfig = {'rhost': hostName, 'ruser': userID, 'rpwd': pWord, 'rdb': dbName, 'dbtype': dbType}
        return aConfig
    elif cStr == 1:
        sConfig = 'mysql+mysqlconnector://' + userID + ':' + pWord + '@' + hostName + '/' + dbName
        return sConfig
...
def uploadRateEGlobe(id):
    # uploadrate = requests.get("https://integrations.revnomix.com/eglobe/outbound/updateRate?clientid="+ str(id))
    uploadrate = requests.get("https://integrations.revnomix.com/eglobe/outbound/updateRate/derived?clientId="+ str(id))
    status = uploadrate.status_code
    return f"Upload completed for clientId: {id} with status code: {status}"
...
def RecommendationService(pid, start_date=None, date_range=None):
    ...
    config = getDBConfig(logger=logger, cArray = 1)
    ...
    dtCreate = datetime.datetime.today()
    prcType = "Recommendation service"

    cnx = connectdb.conectdb(config)
    result = cnx.execute("SELECT hotel_id FROM clients WHERE id ={}".format(pid)).fetchone()
    htlID = result[0]

    try:
        logger.info("Generate the recommendations from %s for next %s ",start_date, date_range)
        update_len, date_range = getRecommendations.getRecommendations(config, pid=pid, start_date=start_date)

        if update_len == "-1":
            logger.info("Failed to generate recommendations from %s for next %s ", start_date, date_range)
            recom_status = "Failed to generate recommendations"
        else:
            logger.info("Generated %s recommendations from %s for next %s ", update_len, start_date, date_range)
            recom_status = ("Generated {} rates".format(update_len))
        ...
        cnx.execute("insert into recom_calib_log (client_id, hotel_id, process_type,process_status, created_date, updated_date, run_datetime) "
                    "VALUES ('{}','{}','{}','{}','{}','{}','{}');".format(pid, htlID, prcType, recom_status, dtCreate, dtCreate, dtCreate))

        # pushstatus = uploadRateCM(pid)
