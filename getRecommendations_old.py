...
def getRecommendations(db_confg, pid, start_date):
    ...
    try:
        ...
        for row in df_bar_all_algo_for_ord.itertuples():

            cnx.execute(f"insert into recommendations_to_show "
                        "(client_id, hotel_id, room_id, checkin_date, strategy, final_rate, "
                        "push_rate, created_date, updated_date) "
                        "VALUES ({}, {}, {}, '{}', '{}', {}, {}, '{}', '{}') "
                        "ON DUPLICATE KEY UPDATE strategy = '{}',  final_rate = {}, push_rate = {}, "
                        "updated_date = '{}' ".format(row.client_id, row.hotel_id, row.room_id, row.checkin_date,
                                                      row.final_algo, row.selected_rate, row.new_rate,
                                                      row.update_date, row.creation_date,
                                                      row.final_algo, row.selected_rate, row.new_rate,
                                                      row.creation_date))
