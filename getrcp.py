...
def getrcp(df_raw, pid, db_confg):
    ...
    # df_rcp['dt1'] = pd.to_datetime(df_rcp['dtyr'].astype(str) + '-' +
    #                             df_rcp['m1'].astype(str) + '-' +
    #                             df_rcp['d1'].astype(str))
    # df_rcp['dt2'] = pd.to_datetime(df_rcp['dtyr'].astype(str) + '-' +
    #                             df_rcp['m1'].astype(str) + '-' +
    #                             df_rcp['d2'].astype(str))
    ...
    df_rcp_season['ratio_top'] = np.where((df_rcp_season['ota_max'] <= 0), 1,
                                          np.where((df_rcp_season['ota_max'] < df_rcp_season['max_cap']),
                                                   (1 - (np.where((df_rcp_season['rm_sold'] > 0),
                                                                df_rcp_season['cm_capacity'],
                                                                df_rcp_season['ota_max'])/df_rcp_season['max_cap'])),
                                                   np.where((df_rcp_season['rm_sold'] > 0),
                                                            (df_rcp_season['rm_sold'] / df_rcp_season['ota_max']), 0)))    
    ...
    df_rcp_season['rcp_delta'] = (((df_rcp_season['rate_dif'] * df_rcp_season['numarator']) /
                                   df_rcp_season['denominator']) * df_rcp_season['ratio_top'])

    df_rcp_season['rcp_raw'] = np.where(df_rcp_season['rcp_delta'] <= 0, df_rcp_season['min_rate'],
                                        df_rcp_season['rcp_delta'] + df_rcp_season['min_rate'])
    ...
    delcols = ['WkNum', 'min_cap', 'max_cap', 'seasonid', 'slope', 'intercept', 'max_rate', 'min_price',
               ...
               'ota_sqrt1', 'ota_sqrt2', 'sqrt0', 'sqrt1', 'sqrt2', 'numarator', 'denominator', 'ratio_top',
               'rcp_delta', 'rcp_raw']
    df_rcp_season = df_rcp_season.drop(delcols, axis=1)
