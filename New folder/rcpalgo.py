...
def rcpalgo(pid, hCap, db_config):
    ...
    if db_config['dbtype'] == 'PGS':
        myquery = "SELECT arrivaldate as CheckInDate, numrooms as Nights, lengthofstay as LOS, " \
                  "totalamount as RoomRevenue FROM transactional_details WHERE propertyid =:prid"
    else:
        myquery = "SELECT checkin_date as CheckInDate, no_of_rooms as Nights, los, total_amount as RoomRevenue FROM " \
                  "bookings WHERE client_id =:prid"
...
def maxcapacity(pid, hCap, db_config):
    qrmcap = ("select t.occupancy_date as occupancydate, sum(t.no_of_rooms) as no_of_rooms"
              ...
              " group by t.occupancy_date".format(pid, ('C', 'M', 'Confirmed', 'Modified', 'New', 'Modify')))
    try:
