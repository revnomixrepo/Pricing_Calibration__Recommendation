...
def getRecommendations(db_confg, pid, start_date):
    ...
    myquery = ("select distinct hotel_code as propertydetailsid, DATE_FORMAT(checkin_date,'%Y-%m-%d') as occupancydate,"
               " " +
               "greatest(onsite_rate,rate) as htl_rate, DATE_FORMAT(date_collected,'%Y-%m-%d') as regdate FROM rm_rates " +
               "where hotel_code in (" + comp_ids +") and checkin_date between '" + start_date + "' and '" + end_date +
               "' and date_collected >='" + clt_date + "' order by hotel_code, occupancydate;")
    ...
    if calc_mpi == "1":
        ...
        logging.debug(mpi_rate.head(n=10))
    ...
    logger.info("--- If PQM is higher than ARI or Lower than MPI then set the average of these two ---")
    ...
    df_bar['rtmpi_b'] = np.where(df_bar['rtmpi_rf'] < df_bar['min_rate'], df_bar['min_rate'] * df_bar['rate_factor'],
                                 df_bar['rtmpi_rf'])
    ...
    logger.info("--- Applying Psychological Factor on PQM Recommendations ---")
    ...
    logging.info("~o~o~o~o~o~o~o~o~o~o~o~o~o DATA TYPE ~o~o~o~o~o~o~o~o~o~o~o~o~o~o~o~o~o~o~o~")
    logging.debug(df_bar.dtypes)
    logging.debug(df_bar.head(n=10))
    logging.info("~o~o~o~o~o~o~o~o~o~o~o~o~o~o~o~o~o~o~o~o~o~o~o~o~o~o~o~o~o~o~o~o~o~o~o~o~o~o~")
    ...
    logger.info("--- Existing BAR Recommendations - %s ---", len(df_bar_raw.index))
    if len(df_bar_raw.index) > 0:
        df_bar_raw['checkin_date'] = pd.to_datetime(df_bar_raw['checkin_date'], format="%Y-%m-%d")
        df_maxdate = pd.DataFrame(df_bar_raw.groupby(['hotel_id', 'client_id', 'room_id',
                                                      'checkin_date'])['creation_date'].max().reset_index())
        merged = df_bar_raw.merge(df_maxdate, on=['hotel_id', 'client_id', 'room_id', 'creation_date',
    ...
    myquery = ("SELECT a1.hotel_id, a1.client_id, a1.room_id, a1.update_date, " +
               ...
               "FROM recommendations_final_by_date GROUP BY client_id, checkin_date) AS a2 " +
               "ON (a1.client_id = a2.client_id AND a1.id = a2.id )  " +
               "WHERE  a1.client_id =:prid  AND  " +
               "a1.checkin_date BETWEEN '" + start_date + "' AND '" + end_date + "' " +
    ...
    try:
        ...
        logger.error("--- Failed to fetch Algo setup. Check the database ---")
    ...
    try:
        if len(df_bar_all_algo_for_ord.index) > 0:
