...
def getRecommendations(db_confg, pid, start_date):
    ...
    if dAlog == "S":
        dict_algo_selcted = run_Inf_Factor.run_inf_factor(df_bar_in,pid,db_confg)
        df_bar_in['default_algo'] = df_bar_in['checkin_date'].map(dict_algo_selcted)
    ...
    try:
        ...
        try:
            df_bar_for_show = df_bar_for_show.drop_duplicates()
        except:
            pass
        ...
        try:
            ...
            for row in df_ord_final.itertuples():
                ...
                cnx.execute("update recommendations_override_by_date set override_value = {} "
                            "where hotel_id ={} and client_id = {} and room_id = {} and "
                            "checkin_date = '{}' and active_override = 'active'".format(float(row.override_value), int(row.hotel_id),
                                                         int(row.client_id), int(row.room_id), row.checkin_date))
